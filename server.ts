 ...
 * Express REST + WebSocket + AutoTrader loop
 */
...
if (!OANDA_KEY || !OANDA_ID || OANDA_ID === 'AUTO') {
  console.error('\n❌  Run: node setup.mjs  to auto-configure your account ID\n');
  process.exit(1);
}

const oanda   = new OANDAService({ apiKey: OANDA_KEY, accountId: OANDA_ID, practice: process.env.OANDA_PRACTICE === 'true' });
const brain   = new BrainEngine(oanda);
const guard   = new PropGuard();
const journal = new TradeJournal();
const ws      = new SovereignWS(parseInt(process.env.WS_PORT || '3001'));
...
app.get('/api/health',  async (_, res) => res.json({ status: 'online', ts: new Date().toISOString(), wsClients: ws.clientCount, trader: trader.getState() }));
app.get('/api/account', async (_, res) => { try { res.json({ ok: true, account: await oanda.getAccount() }); } catch(e:any){ res.status(500).json({ ok:false, error:e.message }); }});
app.get('/api/prices',  async (req, res) => { try { const i=(req.query.i as string||'EUR_USD,GBP_USD,USD_JPY,AUD_USD').split(','); res.json({ ok:true, prices: await oanda.getPrices(i) }); } catch(e:any){ res.status(500).json({ok:false,error:e.message}); }});
app.get('/api/candles/:instrument', async (req,res)=>{ try{ const c=await oanda.getCandles(req.params.instrument,(req.query.gran as any)||'M15',parseInt(req.query.count as string||'100')); res.json({ok:true,candles:c}); }catch(e:any){res.status(500).json({ok:false,error:e.message});}});
app.get('/api/trades',  async (_, res) => { try { res.json({ ok: true, trades: await oanda.getOpenTrades() }); } catch(e:any){ res.status(500).json({ ok:false, error:e.message }); }});
app.get('/api/guards',  async (_, res) => { try { const a=await oanda.getAccount(); res.json({ ok:true, ...guard.check(a) }); } catch(e:any){ res.status(500).json({ok:false,error:e.message}); }});
app.get('/api/journal', async (_, res) => { try { const [e,s]=await Promise.all([journal.getAll(),journal.getWeeklySummary()]); res.json({ok:true,entries:e,summary:s}); } catch(e:any){ res.status(500).json({ok:false,error:e.message}); }});
app.get('/api/summary', async (_, res) => { try { const [a,s]=await Promise.all([oanda.getAccount(),journal.getWeeklySummary()]); res.json({ok:true,account:a,summary:s,traderState:trader.getState()}); } catch(e:any){ res.status(500).json({ok:false,error:e.message}); }});
app.get('/api/trader/state', (_, res) => res.json({ ok: true, state: trader.getState() }));

app.post('/api/calc-lot', async (req, res) => {
  try { const r = await oanda.calcLotSize(req.body); res.json({ ok: true, result: r }); }
...
app.delete('/api/trade/:id', async (req, res) => {
  try { const pnl=await oanda.closeTrade(req.params.id); ws.emitTradeClosed({id:req.params.id,instrument:'',pnl,exitPrice:0,reason:'Manual'}); res.json({ok:true,pnl}); }
  catch(e:any){ res.status(500).json({ok:false,error:e.message}); }
});
...
app.listen(PORT, () => {
  ...
  console.log(`   WebSocket : ws://localhost:${process.env.WS_PORT||3001}`);
